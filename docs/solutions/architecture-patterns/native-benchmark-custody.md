...
4. **Let the child inherit CPU affinity without a launcher or `preexec_fn`.** Save the caller thread's affinity. Set the campaign affinity before `Popen`, then restore the caller in `finally`. The child inherits the temporary mask. This removes PATH launcher substitution and avoids Python's unsafe post-fork `preexec_fn` path. Restore the mask when spawning fails. If restoration fails after the child starts, terminate and reap the child.
