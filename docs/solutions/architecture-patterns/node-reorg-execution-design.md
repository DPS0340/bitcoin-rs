...
| Real crash replay / recovery | In-flight marker detection, startup refusal, and clean-tip convergence verified across backends; see [docs/contracts/chain-events.md](../../contracts/chain-events.md). The G11 gate (`bin/bitcoin-rs/tests/gates/g11_crash_recovery.rs`) proves the recovery-meta sidecar protocol (simulated interrupted apply, atomic write, torn-meta refusal, stale-tmp tolerance) across RocksDB, fjall, and redb, plus the recovery-evidence bounded current/previous file protocol (semantic rotation, foreign-genesis refusal, oversized-file refusal). It does not exercise full-stack `kill -9` of a running node or real block-body re-application through the UTXO listener. |
