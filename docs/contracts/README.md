...
| [mempool-policy.md](mempool-policy.md) | `POL-01`–`POL-06` | Admission owner, policy pins, preview/finality, replacement and package rules | policy/RBF suites plus planned admission/finality/cluster suites |
 | [external-api.md](external-api.md) | `API-01`–`API-31` | RPC/REST/ZMQ manifest, errors, mining RPCs, reference provenance, BIP22/BIP23 template extras, mainnet GBT operational gates, `submitheader`, GBT client-rule negotiation, `submitblock` decode, GBT proposal request parsing, `submitblock` uncommitted witness fill, Core v31 submit/proposal duplicate vocabulary, BIP22 reject-reason mapping, GBT `vbrequired` always 0, Core `CheckWitnessMalleation` reject reasons, GBT `coinbaseaux.flags` empty hex, `prioritisetransaction` dummy/`fee_delta` arity, pooled-dust refusal, `getmininginfo` omitted optional fields, and `estimatesmartfee` conf_target/estimate_mode gates, and `generateblock` txid/raw-tx parse errors, and `getprioritisedtransactions` modified_fee in satoshis, and generate invalid-output Core -5 text, and `generateblock` `TestBlockValidity` before solve, and `generateblock` multipath/ranged/Expand private-key errors | manifest coverage, Core parity, mining tests, GBT BIP23 extras, gate, submitheader, negotiation, prioritisetransaction, witness malleation, getmininginfo omission, and estimatesmartfee validation tests, and generateblock parse-error tests |
...
| [p2p-wire.md](p2p-wire.md) | `P2P-01`–`P2P-05` | Wire compatibility, peer leases, demonstrated capability, canonical frontier recovery, connected-socket posture | P2P compatibility/live interop, peer-table and sync tests |
...
| [embedding.md](embedding.md) | `EMB-01`–`EMB-10` | Embedded lifecycle, shared node services and independent stall evidence | embedding/teardown tests; offline watchdog regressions |
