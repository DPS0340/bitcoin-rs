...
| Node `apply::consensus_rule_tests` fixture tree | Delete the node-level duplicate consensus/apply matrix. Consensus rules stay with consensus/script/vector owners; authoritative mutation/recovery tests move to `bitcoin-rs-chainstate`; node keeps only cross-domain behavior. |
