...
| Policy script checks | ATMP `PolicyScriptChecks`: executes input scripts under `STANDARD_SCRIPT_VERIFY_FLAGS` before admitting to the pool | `MempoolGateway::admit_transaction` (step 4b) and `accept::evaluate_package_acceptance_all` run `verify_transaction` with `VerifyFlags::STANDARD` against resolved UTXO and mempool inputs before mutation; script-invalid transactions fail closed (`AdmitError::Consensus` / `consensus-verification-failed`) | implemented | `admit_transaction_rejects_a_script_invalid_input` (`gateway.rs` unit tests) |
