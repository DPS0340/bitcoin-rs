...
| Duplicate submission | Re-submitting a known tx succeeds without error | Same: `sendrawtransaction` returns the txid when the transaction is already in the pool; preview reports `txn-already-in-mempool`. A transaction that has left the pool is re-evaluated, not treated as still admitted. | implemented | `transaction_methods.rs::sendrawtransaction_idempotent_for_already_in_mempool`, `handlers/tx.rs::sendrawtransaction_does_not_treat_an_evicted_tx_as_already_known` |
...
- **Supplementary vectors**: `cargo test -p bitcoin-rs-mempool --test rbf_bip125` (per-rule RBF table incl. rule 5) and the `standardness.rs` unit tests (per-check standardness vectors).
