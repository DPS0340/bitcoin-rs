...
This policy applies to the transaction-acceptance surface of `bitcoin-rs-mempool` (`crates/mempool`: `standardness.rs`, `policy.rs`, `rbf.rs`, `eviction.rs`) and its two admission outlets — the pool mutation API and the RPC handlers `sendrawtransaction` / `testmempoolaccept` (`crates/rpc/src/handlers/tx.rs`). It covers relay policy only: rules a full node may enforce without breaking consensus. Consensus validation of scripts and sighashes is out of scope here (see `docs/policies/core-compatibility.md` and the consensus tests).
...
| Duplicate submission | A tx already in the mempool is not re-submitted (`node/transaction.cpp` `BroadcastTransaction`: "There's already a transaction in the mempool with this txid. Don't try to submit this transaction to the mempool"); a tx not in the mempool is processed again ("Transaction is not already in the mempool.") | Same distinction: `sendrawtransaction` returns the txid for a current pool hit without re-admission; a process-local `ctx.transactions` hit after eviction is not treated as known and goes through admission again. Preview reports `txn-already-in-mempool` only for a pool hit | implemented | `transaction_methods.rs::sendrawtransaction_idempotent_for_already_in_mempool` (pool hit), `transaction_methods.rs::sendrawtransaction_readmits_a_transaction_evicted_from_the_mempool` (evicted resubmission) |
...
- **Supplementary vectors**: `cargo test -p bitcoin-rs-mempool --test rbf_bip125` (per-rule RBF table incl. rule 5), the `standardness.rs` unit tests (per-check standardness vectors), and `cargo test -p bitcoin-rs-rpc --test transaction_methods` (POL-01 duplicate submission: in-mempool idempotency and evicted resubmission).
