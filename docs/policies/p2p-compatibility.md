...
| `sendcmpct` | negotiated | BIP152. Sent after `verack` in low-bandwidth mode (`send_compact=false`, version 2): we never take a high-bandwidth announcement slot, so compact blocks arrive only when we request them. A known-version inbound announcement (v1 or v2) publishes compact-fetch eligibility for that connection regardless of its high-bandwidth flag — an inbound peer is never selected for push announcements. Reconstruction matches short IDs under the identity profile we advertised (`COMPACT_BLOCK_VERSION`), which is the profile a compliant peer uses toward us. |
| `cmpctblock` / `blocktxn` | sink | BIP152 receive path. `cmpctblock` starts bounded per-peer reconstruction: the prefilled coinbase/transactions plus short-ID matches against the mempool under the identity profile we advertised (`COMPACT_BLOCK_VERSION`) — BIP152 identity is directional: our advertised version fixes what a compliant peer sends us, and the peer's recorded version fixes what we serve; missing transactions are requested with `getblocktxn` on the same connection; a reconstruction that stays incomplete (unknown short IDs, ambiguity, bounds, or deadline) falls back to one full-block `getdata`. `blocktxn` completes a pending reconstruction, and the finished block enters the ordinary block pipeline like any `block` message — no validation bypass. |
| `getblocktxn` | served | BIP152. Answered with `blocktxn` from the active chain under the same body-availability and headroom rules as full-block `getdata`; an out-of-range transaction index is a protocol disconnect (Core scores misbehavior). |
