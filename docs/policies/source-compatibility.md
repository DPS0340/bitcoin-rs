...
- Prohibited dependencies: `tokio`, `async-std`, or any async runtime. The node architecture uses a synchronous crossbeam-channel event loop (the historical implementation plan).
...
`bitcoin-rs` operates on a strict **clean cutover** principle (the historical implementation plan). The project rejects:
