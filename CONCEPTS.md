...
The standard node operational configuration tuned for mainnet sync: `fjall` storage backend, multi-peer block download active (outbound peer target 8, pending block budget 128, 16 in-flight requests per peer), hash-pinned assume-valid active on mainnet (height 938343), 450 MiB database cache (`dbcache`, matching Bitcoin Core parity), automatic pruning enabled with a 550 MiB target and a 288-block reorg-safety floor, while secondary indexes (`txindex`, `blockfilterindex`) and `utreexo` stateless validation remain disabled by default. Setting `prune_target_mb` to 0 opts back into archival block storage.
...
### Platform shutdown bridge
The cross-platform `ctrlc` handler that converts process termination input into the node's shared shutdown channel. With the crate's `termination` feature, Unix targets handle SIGINT and SIGTERM while Windows targets handle Ctrl+C and Ctrl+Break without node-owned platform branches. Checkpoint and subsystem draining remain in the common shutdown lifecycle rather than the signal callback.

### Timeout-resumable P2P framing
The stateful Bitcoin v1 message reader used after handshake. TCP read timeouts are polling events, not frame boundaries: bytes already consumed before a timeout remain buffered and the next poll resumes the same header or payload. Without this state, a large block split by a timeout causes payload bytes to be misread as the next network magic and disconnects a valid peer with a random-looking magic mismatch.

### Node network selection
The user-facing `BITCOIN_RS_NETWORK`/`--network` selection that atomically supplies consensus rules and P2P bootstrap identity while preserving later, low-level overrides. Standard Bitcoin names use their matching consensus `Network`, message start, and DNS bootstrap. `drynet4` shares mainnet history through height 961631, applies the fork's one-time difficulty reset to mainnet PoW limit `1d00ffff` at height 961632, uses message start `eca5d404`, disables Bitcoin DNS seeds, and connects to `drynet4.drivechain.dev:8533`. The reset is carried through header sync, block apply, and checkpoint restore; later retargets use normal mainnet rules. Compose passes the same selection to bitcoin-rs and the BIP300/301 enforcer and uses it to namespace their data directories. The internal base `Network` remains `mainnet` for drynet4.
