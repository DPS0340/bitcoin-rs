...
Parsing each block exactly once on the apply path and reusing that parse downstream for txids and the transaction objects script preparation borrows via `TransactionRef`. `kernel::BlockParse` (`crates/consensus/src/kernel.rs`) is the one-shot parse; its parse engine follows the selected `validation.engine`, so `Native` parses with the Rust parser and `Kernel` with `bitcoinkernel::Block::new`. Both variants are compiled in every build (`kernel` builds also compile the bitcoinkernel backend). Price a replacement by everything it subsumes, not by the line item that motivated it.
