...
### bitcoinconsensus
Bitcoin Core's script-verification engine, extracted as a C library and linked into bitcoin-rs as the default backend for **non-taproot** script verification; it performs the per-input signature and script checks (for non-taproot inputs) that assumevalid skips below the trusted height. Non-taproot script evaluation delegates to `bitcoinconsensus`; this narrows likely Rust-side leverage but does not rule out a measured cache hit or wrapper/data-preparation win. Taproot inputs instead run the parallel Rust validation path's interpreter even on the default build; any proposed optimization requires profiling and benchmarking actual hit rates or wrapper costs.
