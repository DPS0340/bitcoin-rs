...
Authoritative C150/Cmodern certification requires file-bound binary streams, strict `mainnet-prefix-replay-validation-v1` inputs, and exact classifier validation. Direct Core REST export can export raw blocks prior to replay, but live REST export cannot replace file-bound census evidence for certification. Sampled evidence (such as `kernel_verify_spike`) cannot certify a product corpus.
