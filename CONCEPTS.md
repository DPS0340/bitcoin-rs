...
### bitcoinkernel
Bitcoin Core's C++ consensus engine (`libbitcoinkernel`), compiled into `bitcoin-rs` as the production consensus engine. The `kernel` feature is the default in `crates/consensus` and `crates/node`, but **not** in `bin/bitcoin-rs` (whose defaults are `["fjall", "redb", "zmq"]`), so `cargo build -p bitcoin-rs` excludes `bitcoinkernel` from its dependency graph. Beyond script verification it is also the block **parser** on the apply path — see *One-shot kernel block parse*. It validates input scripts across all script classes (legacy, segwit, and Taproot key-path and script-path spends). Builds with `kernel` require system dependencies (`cmake` and `libboost-dev`). Production transaction and block input-script verification route to bitcoinkernel when the `kernel` feature is enabled, while Rust performs surrounding non-script transaction and block consensus checks; the Rust `Interpreter` remains a separate portable script-verification surface under `--no-default-features` (see *Rust interpreter (portable posture)*). The #166 kernel flip — making `kernel` opt-in across all crates — is blocked until the portable interpreter can validate ordinary spends (see *Rust interpreter (portable posture)*).
### bitcoinconsensus
