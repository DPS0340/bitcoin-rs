...
pub(crate) fn user_config_from_env(
    ...
    for (key, value) in vars {
        ...
        if apply_zmq_env(&mut layer.zmq, key, value)? {
            continue;
        }
        match key {
            "BITCOIN_RS_NETWORK" => {
            ...
            "BITCOIN_RS_DNS_SEEDS_ENABLED" => {
                layer.p2p.dns_seeds = Some(parse_bool(value)?);
            }
...
fn apply_zmq_env(layer: &mut ZmqOverrides, key: &str, value: &str) -> Result<bool> {
