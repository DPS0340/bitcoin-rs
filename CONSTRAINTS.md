...
Target values are contracts, not results. Unknown is not false; a missing
measurement blocks the owning task.
...
to go green on hosted runners. Proof acceptance requires all six pinned
Apalache checks to finish successfully on a runner with sufficient resources.
...
Every row remains **UNMEASURED** for baseline, final, verdict, and evidence.
These rules and targets are requirements, not successful measurements.

The scope column distinguishes diff-local obligations from whole-project
claims; historical task IDs identify the original evidence plan. The owning
contracts define each surface's proof. Use `scripts/ci-pr.sh` for
Cargo gates and the pinned runner above for formal checks. Removed historical
test targets and task schedules are not a second validation system. A local
suite passing does not establish a full-tip, cross-platform, or performance
claim; those rows still require their independent measurements and identities.

| Constraint | Rule | Scope | Target and source |
| --- | --- | --- | --- |
| CL-01: Complete coverage | T40 dependency closure reaches all 38 remaining tasks T00-T27, T29-T32, T35-T40, every remaining requirement R001-R023, R025-R034, R036-R064 has successful required command evidence, and MERKLE-ALL is closed; no partial-owner or exception shortcut counts as whole delivery. | Project-wide delivery | Required true; plan internal dependencies and `REQUIREMENTS.csv` |
| CL-02: Strict-Rust mandatory | T16 strict-Rust cryptography succeeds before T17 default promotion; final comparator uses the strict candidate; missing strict evidence blocks promotion. | Project-wide validation closure; T16, T17, T40 | Required true; Approval effects, R048 |
| CL-03: Kernel independence | No `bitcoinkernel` in the production transitive graph; native candidate and independent oracle use separate `CARGO_TARGET_DIR` and independently identified artifacts. Preserve dependency-direction and default-validation checks. | Project-wide production dependency closure; T17, T40 | Required true; Verification 0/5, R004/R006/R049 |
| CL-04: 1 TB physical peak | Default unpruned fjall, indexes off, pinned full-tip stop: `conservative_high_water` and `PhysicalLedger::data_directory_allocated_bytes` prove actual physical peak, including transient allocations; physical bytes also do not exceed T02 baseline. Missing original-candidate high-water blocks T14 cut and regression proof. | Project-wide full-tip physical truth; T02, T14, T39, T40 | Physical peak <= 1_000_000_000_000 decimal bytes and <= baseline; FP-02/FP-04, R052 |
| CL-05: Separate ledgers | Emit logical and physical ledgers separately; never sum them or use logical payload as allocated-storage truth. Budget and peak verdict derive from the physical ledger only. | Diff: storage accounting and changed I/O; T14, T39 | Required true; FP-01 |
| CL-06: Parser/protocol equality | Independent native and oracle results agree on txids, wtxids, weight, positions, and Merkle flag; T15 has zero unexplained contextual or script mismatches. Preserve previously passing contracts. | Diff: parser, consensus, script and affected I/O; T06, T15 | Required true and no unexplained mismatches; R002/R005 |
| CL-07: Coherent views | Every mixed read carries a `chain_generation` token; a height-only cache is a defect. RPC, REST, ZMQ, Esplora, and backend projections preserve coherent authoritative snapshots. | Diff: mixed reads and public projections; T22, T31, T32 | Required true; BLUEPRINT INV-05/INV-08 |
| CL-08: Single owners | Five-layer graph satisfies `g17`; one admission owner `MempoolGateway`, scheduler `P2pService`, mempool orphan owner, template owner `MiningCoordinator`, and index runtime owner. Projections are disposable and rebuildable, never alternate authority. | Diff: ownership and dependency cuts; T03, T18, T22, T24, T29, T35 | Required true; INV-02/INV-03, R012/R013/R046/R047 |
| CL-09: Immutable pins | `ReferenceSet` records independent source, binary, and corpus digests; pin toolchain `1.95.0` and `Cargo.lock` SHA-256. Candidate-derived expected answers are not independent references. | Project-wide evidence and reference identity; T00 | Required true; T00, Verification 0, FP-03 |
| CL-10: Actual external consumers | The external miner over public GBT requires its owner-gate evidence. In-process shortcuts do not prove the consumer contract. | Diff: external-miner contract; T36 | Required true; R043, Verification 4 |
| CL-11: Full-platform Merkle | Portable code compiles across the full matrix; record measured, compile-only, or unavailable status without inventing verdicts. Guard AVX2, x86 SHA, and ARM sha2; unsupported instructions never execute. Missing hardware blocks its performance proof, not its correctness obligation. | Project-wide MERKLE-ALL platform matrix; T02, T37 | Required correctness true; matched per-target throughput >= baseline and cost/latency <= baseline; Verification 6 |
| CL-12: Explicit fresh replay | Increment owner-local `CURRENT_SCHEMA`; incompatible data refuses open with `incompatible_schema`. No translator or legacy reader. Fresh replay uses disposable candidate data, never an operator-data write or hidden repair. T14 authority cut requires original physical baseline first. | Diff: authoritative format and open path; T12, T14 | Required true; Verification 0, Assumptions |
| CL-13: No old readers / shims | Each approved deletion cut displays its removed set; no re-export, alias, forwarding path, compatibility flag, or old reader preserves an approved-deleted symbol. Owner-local cuts cannot create a second policy or authority. | Diff: approved deletions and dependents; T03-T40 | Required true; Approach anti-shim |
| CL-14: Bounded budgets | Every ingress, window, traversal, response, cache, journal, and rebuild has an owner and resolved byte, work, and time bounds. Include window bytes/inputs/coin/CPU/age; `MempoolLimits` cluster_count 64, size 101_000 vB in exact adjusted weight, max_replacement_clusters 100, max_total_bytes 300_000_000; `DEFAULT_TX_RELAY_QUEUE_CAPACITY` 1024; `DEFAULT_ZMQ_HWM` 1_000; owner orphan quota and `PrefixScanLimit`. Capture RSS and retained-byte high-water for each affected existing surface. | Diff: affected resource cells; T02, T06, T08, T13, T14, T20, T22, T25, T38, T39 | Usage <= actual resolved owner limits and applicable T02 baseline; INV-10 |
| CL-15: Four attempts / 16 000 sigops | Shared admission attempts at most four times (`MAX_ADMISSION_RETRIES`) then returns typed `RetryExhausted`; `total_sigop_cost` <= `MAX_STANDARD_TX_SIGOPS_COST`. Resolve owner constants rather than adding knobs. | Diff: admission; T18 | Required true; retained constants 4 and 16_000, T18 |
| CL-16: Selected Core 31.1 policy | Selected process settings: min-relay 1_000 sat/kvB, incremental 1_000, dust 3_000, datacarrier 83; package count 25, cluster count 64 and direct-conflict cluster count 100. These relay rates are explicit settings, not Core 31.1 defaults. Supported replacement/preview/TRUC behavior and intentional unsupported cases are declared in the compatibility manifest; resolved startup `AdmissionPolicy` remains a target. | Diff: policy and replacement contracts; T19, T21 | Required true; `docs/policies/mempool-policy.md` |
| CL-17: Durable order | Append body/undo, sync, batch, durable completion, publication. `submitblock` succeeds only after durability; I/O failure never means success. `DurableHead.commit_id` is monotonic; backend atomicity and crash evidence are mandatory. | Diff: durable write and publication surfaces; T09, T11, T12 | Required true; INV-04/INV-06 |
| CL-18: Exact inverse | Disconnect restores pre-connect coins, bookkeeping, and identity; missing undo fails closed. Streaming reorg and pruning leases preserve boundedness and the existing I/O baseline. | Diff: disconnect and reorg; T13 | Required true; INV-07, T13 |
| CL-19: Performance gain | Promotion requires median gain >= 1.05x, at least 3 alternating runs, each arm within 5% stability, improvement exceeding host noise, and identical result hashes. Capture original `sync_pipeline`, `chainstate_journal`, and `merkle` controls at T02; include microbenchmarks and full E2E product workloads. | Diff: proposed optimizations and affected product paths; T02, T37, T38 | Gain threshold plus all non-regression predicates; Verification 6, R064 |
| CL-20: No regression | Every applicable apply-latency, cost, RSS, retained-byte, storage, p99, and throughput cell satisfies its baseline direction; preserve Boolean contracts. Hold script backend constant. 3% median and 5% p99 outer rejection caps never authorize degradation; uncertain noise comparisons remain blocked. | Diff: every affected measured surface; T02, T06, T08, T14, T37, T38, T39 | Cost/latency/bytes <= baseline; throughput >= baseline; R005/R064, Verification 6 |
| CL-21: No unapproved surface | A public boundary outside Approval effects stops the cut pending separate human approval. No unapproved limit, config, or crate beyond approved `crates/chainstate`. No writes to operator data; test and replay writes are confined to disposable isolated fixtures. | Diff: all changed boundaries, dependencies, configuration and I/O | Required true; Approval effects |
| CL-22: Version/lock discipline | Approved workspace 0.7.0 to 0.8.0 release change and `Cargo.lock` update are atomic. Schema and projection versions stay owner-local; disposable projections never dictate authoritative schema. Commands use `--locked`. | Diff: owner-local versions; project-wide release, lock, and dependency closure; T40 | Required true; `docs/policies/source-compatibility.md` section 4 |
| CL-23: Honest failure | Unavailable is not empty; unverified is not supported; unknown is not false. Skipped or unavailable proof blocks its gate with the missing identity recorded; numeric and Boolean fields remain UNMEASURED until measured. | Diff: affected evidence and failure surfaces across all tasks | Required true; typed `Unavailable`/`BLOCKED`, INV-11, Verification 0 |

## Record discipline

- Preserve every CL ID and its requirement. Record measurements per surface,
  platform, or workload with the exact command and complete evidence identity.
  Keep run results in the PR or CI artifacts, not duplicate status columns here.
