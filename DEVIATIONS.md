...
- `getmempoolinfo` returns real `size`, `bytes`, `total_fee` numbers via `Mempool::stats()`, and `usage` is `Mempool::dynamic_memory_usage()` — the pool's estimated heap, Bitcoin Core's `DynamicMemoryUsage()`. It is an estimate of *this node's* structures, not a reproduction of Core's number, the way Core's own figure estimates boost's containers. The arena and the txid map are charged at allocated capacity, because neither returns its allocation on removal or on `clear` — a drained pool is still holding that memory and says so, rather than reporting zero at the moment someone asks where it went. **`maxmempool` still bounds the virtual-size sum, not `usage`**, so unlike Core the two are not the same currency and `usage` may exceed `maxmempool`; changing what the limit bounds is a policy change and has not been made.
