...
- **`getblockchaininfo`'s `verificationprogress` is still a height ratio (`applied / headers`), not Core's `GuessVerificationProgress`.** Early blocks are nearly empty, so it reports the chain further along than it is. Core's formula is a ratio of cumulative transaction counts, and it needs an `nChainTx` that survives a restart; this node has none. The RPC block-record log is the only place transaction counts live and `NodeState` rebuilds it empty on every open, so the count reads zero on any node that did not sync in the current process. Correcting the formula before making its input durable would replace a wrong-but-close number with a confident `0.0` on every restarted node. The same gap caps `getchaintxstats`'s `txcount`.
