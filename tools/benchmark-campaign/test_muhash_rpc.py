...
class CampaignControllerTests(unittest.TestCase):
    ...
    def test_snapshot_falls_back_when_exec_flags_are_einval(self) -> None:
        # Contract clause: docs/contracts/muhash-rpc.md MRPC-03.
        # memfd_create(2) returns EINVAL for unknown flag bits; Linux 6.3
        # introduced MFD_EXEC and MFD_NOEXEC_SEAL.
        root = _make_root("muhash-memfd-einval-")
        script = b"#!/usr/bin/env python3\nimport sys\nsys.stdout.write('ran')\n"
        binary = root / "node.py"
        binary.write_bytes(script)
        binary.chmod(0o700)
        ...
        config.write_bytes(b"pinned-config")
        calls: list[int] = []
        real_memfd = module.os.memfd_create

        def hijack(name: str, flags: int = 0) -> int:
            calls.append(flags)
            if flags != module._MFD_ALLOW_SEALING:
                raise OSError(errno.EINVAL, "Invalid argument")
            return real_memfd(name, flags)

        binary_fd = -1
        config_fd = -1
        module.os.memfd_create = hijack  # type: ignore[method-assign]
        try:
            try:
                binary_fd = module._open_verified_snapshot(
                    binary,
                ...
                module.os.memfd_create = real_memfd  # type: ignore[method-assign]
            self.assertEqual(
                calls,
                [
                    module._MFD_ALLOW_SEALING | module._MFD_EXEC,
                    module._MFD_ALLOW_SEALING,
                    module._MFD_ALLOW_SEALING | module._MFD_NOEXEC_SEAL,
                    module._MFD_ALLOW_SEALING,
                ],
            )
            ran = subprocess.run(
