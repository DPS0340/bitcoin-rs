...
- Implemented RPCs (crates/rpc/src/registry.rs): `getblockcount`, `getbestblockhash`, `getpeerinfo`, `getconnectioncount`, `getblockchaininfo`, `submitblock`. There is no `generatetoaddress` — build/mine blocks yourself (see below).
...
- Frame decoding caps: `ProcessPeer`'s read helper caps payloads at 4 MB — the node can legitimately send larger `headers` batches (each header is 80 bytes); use your own reader with a ≥32 MB cap.
