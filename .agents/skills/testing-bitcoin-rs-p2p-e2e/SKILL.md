...
- To make the node dial a fake peer (covers `run_outbound_handshake`/`send_handshake_message` + outbound writer loop): pre-bind a `TcpListener` on `127.0.0.1:0`, then `ProcessNode::start_with_options(NodeBinary::BitcoinRs, &["--connect", &addr], START_TIMEOUT)`. `connect` becomes `fixed_peers` → `run_fixed_peer_bootstrap` pushes it to the outbound-drain worker, which dials within ~1 s. `accept()` with a nonblocking poll loop (30 s) catches it.
