...
impl<S: KvStore> UndoStore for KvUndoStore<S> {
    /// Writes without `flush`, deliberately.
    ///
    /// No part of the apply path fsyncs per block: the UTXO commit beside this
    /// write is journaled too, and `crash_recovery` replays the gap from its
    /// watermark on boot. Syncing only the undo record would buy nothing (the
    /// commit it describes could still be lost) and would put an fsync on every
    /// connected block. Undo records are regenerated by that replay, because
    /// they are derived from the block and the pre-block UTXO state.
    fn persist_undo(
        &self,
        height: u32,
        hash: bitcoin_rs_primitives::Hash256,
