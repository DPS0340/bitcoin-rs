...
#![expect(clippy::expect_used, reason = "test: lifecycle unit tests use expect for clarity")]
...
fn publication_boundaries_never_expose_half_installed_state() {
    ...
    assert!(matches!(
        **snapshot,
        TxIndexLifecycle::ShutdownAbandoned
    ));

    // Publish Failed.
    lifecycle.store(Arc::new(TxIndexLifecycle::Failed(
...
fn stale_generation_rcu_is_a_noop() {
    ...
    publish_failed_if_current(&lifecycle, &gen, "should not publish");
    let snapshot = lifecycle.load();
    assert!(
...
fn query_adapter_returns_unavailable_for_opening() {
    ...
    let result = adapter.transaction(&Txid::all_zeros());
    assert!(
...
fn namespace_registry_claims_and_releases() {
    ...
    assert!(matches!(
        registry.entries.lock().expect("lock").get(&key),
