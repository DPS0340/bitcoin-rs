...
fn build_tx_index_open_spec(
    ...
    Ok(Some(crate::txindex_worker::TxIndexOpenSpec {
        ...
        epoch,
        enabled,
        rollback_rebuild_cutover: config.indexes.rollback_rebuild_cutover,
        canonical_data_root,
    }))
}
...
impl NodeState {
    ...
    pub fn open(
        ...
        let active_zmq_notifications: Vec<_> = config
            .zmq
            .iter()
            .map(|publication| {
                ZmqNotification::new(
            ...
            .collect();
        #[cfg(feature = "zmq")]
        let zmq_publisher: Arc<dyn crate::ZmqPublisher> = if config.zmq.is_empty() {
            Arc::new(crate::NoOpZmqPublisher)
        } else {
            Arc::new(crate::SocketZmqPublisher::bind(&config.zmq)?)
        };
        #[cfg(not(feature = "zmq"))]
...
mod tests {
    ...
    fn zmq_publisher_handle_reports_active_metadata() -> anyhow::Result<()> {
        ...
        config.p2p.listen.clear();
        config.zmq = vec![
            crate::ZmqPublication {
                topic: crate::zmq_publisher::ZmqTopic::HashBlock,
                endpoint: "inproc://state-zmq-pubhashblock".to_owned(),
                hwm: 17,
            },
            crate::ZmqPublication {
                topic: crate::zmq_publisher::ZmqTopic::HashTx,
                endpoint: "inproc://state-zmq-pubhashtx".to_owned(),
                hwm: 18,
            },
            crate::ZmqPublication {
                topic: crate::zmq_publisher::ZmqTopic::RawBlock,
                endpoint: "inproc://state-zmq-pubrawblock".to_owned(),
                hwm: 19,
            },
            crate::ZmqPublication {
                topic: crate::zmq_publisher::ZmqTopic::RawTx,
                endpoint: "inproc://state-zmq-pubrawtx".to_owned(),
                hwm: 20,
            },
        ];
        let state = NodeState::open(config, None)?;
