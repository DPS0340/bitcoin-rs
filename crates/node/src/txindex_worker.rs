...
use bitcoin_rs_index::{
    ...
    PreparedBatchLimits, PreparedBlock, ScriptHash, TxIndexScan, TxIndexSnapshot,
};
...
const QUERY_BLOCK_LOAD_LIMIT: usize = 4_096;
...
impl QueryBudget {
    const fn new() -> Self {
        Self {
            ...
            remaining_block_loads: QUERY_BLOCK_LOAD_LIMIT,
        }
    }
    ...
    fn accept_scan(&mut self, scan: TxIndexScan) -> Result<Vec<HashPrefixRow>, TxQueryError> {
        if !scan.complete {
            return Err(TxQueryError::Unavailable(
    ...
    fn reserve_block_load(&mut self) -> Result<(), TxQueryError> {
        if self.remaining_block_loads == 0 || self.remaining_bytes < MAX_SERIALIZED_BLOCK_BYTES {
            return Err(TxQueryError::Unavailable(
                "txindex query block budget exhausted".into(),
            ));
        }
        self.remaining_block_loads -= 1;
        Ok(())
    }

    fn charge_block_bytes(&mut self, bytes: usize) -> Result<(), TxQueryError> {
        if bytes > self.remaining_bytes {
            return Err(TxQueryError::Unavailable(
                "txindex query block budget exceeded".into(),
            ));
        }
...
impl TxIndexQueryEngine {
    ...
    fn resolve_block(
        ...
        budget.reserve_block_load()?;
        let bytes = self.resolve_block_body_bytes(height, hash)?;
        budget.charge_block_bytes(bytes.len())?;
    ...
    fn transaction_for(
        ...
        for row in rows {
            ...
            if last_height != Some(height) {
                ...
                cached_block = Some(self.resolve_block(budget, height, hash)?);
                last_height = Some(height);
            }
            let block = cached_block.as_ref().ok_or_else(|| {
                TxQueryError::Unavailable("missing block during txid query".into())
            })?;
    ...
    fn confirmed_history_snapshot_for(
        ...
        let funding_rows = Self::scan_funding_rows(snapshot, budget, scripthash)?;

        let mut history = Vec::new();
        let mut unspent = Vec::new();
        // Funding outputs that need one spending scan to complete both answers.
        let mut funding_outputs: Vec<(Txid, u32, u64, u32)> = Vec::new();
        ...
        for row in funding_rows {
            ...
            for tx in &block.txdata {
                ...
                for (vout_idx, output) in tx.output.iter().enumerate() {
                    ...
                    funding_outputs.push((txid, vout, value, height));
                }
            }
        }
                ...
                for (vout_idx, output) in tx.output.iter().enumerate() {
                    ...
                    for row in spend_rows {
                        ...
                        if last_spend_height != Some(spend_height) {
                            let hash = self.resolve_hash_at_height(spend_height, tip)?;
                            cached_spend_block =
                                Some(self.resolve_block(budget, spend_height, hash)?);
                            last_spend_height = Some(spend_height);
                        }
                        let block = cached_spend_block.as_ref().ok_or_else(|| {
                            TxQueryError::Unavailable("missing block during unspent query".into())
