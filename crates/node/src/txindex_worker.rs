...
pub(crate) struct NamespaceRegistry {
    entries: std::sync::Mutex<HashMap<PathBuf, NamespaceEntry>>,
}
 ...
 /// `.` or `..`, not absolute. Does not canonicalize the child.
    fn validate_child(root: &Path, child: &str) -> Result<PathBuf, String> {
        ...
        if child.contains(std::path::MAIN_SEPARATOR) {
            return Err(format!(
                "namespace child {child} contains a path separator"
            ));
        }
        if child == "." || child == ".." {
    ...
    fn claim(&self, key: PathBuf, owner: u64) -> bool {
        let mut entries = self.entries.lock().expect("namespace registry lock");
        match entries.get(&key) {
            None => {
                entries.insert(key, NamespaceEntry::Active(owner));
                true
            }
            Some(NamespaceEntry::Active(_)) | Some(NamespaceEntry::Poisoned) => false,
        }
    }
    ...
    fn is_poisoned(&self, key: &Path) -> bool {
        matches!(
            self.entries.lock().expect("namespace registry lock").get(key),
            Some(NamespaceEntry::Poisoned)
        )
    }
}
...
pub(crate) static TXINDEX_OPEN_GATE: std::sync::LazyLock<
    std::sync::Mutex<Option<crossbeam_channel::Receiver<()>>>,
> = std::sync::LazyLock::new(|| std::sync::Mutex::new(None));

#[cfg(test)]
pub(crate) fn install_txindex_open_gate() -> crossbeam_channel::Sender<()> {
    let (tx, rx) = crossbeam_channel::bounded(1);
    *TXINDEX_OPEN_GATE.lock().expect("open gate lock") = Some(rx);
    tx
}

#[cfg(test)]
pub(crate) fn wait_txindex_open_gate() {
    if let Some(rx) = TXINDEX_OPEN_GATE.lock().expect("open gate lock").as_ref() {
        let _ = rx.recv();
    }
}
...
fn run_worker_with_open(
    ...
    let namespace_key = match NamespaceRegistry::validate_child(
        &spec.canonical_data_root,
        spec.namespace,
    ) {
        Ok(key) => key,
        Err(reason) => {
...
fn open_tx_index_on_worker(
    ...
    match storage_backend {
        ...
        "rocksdb" => {
            ...
            Ok(open_tx_index_store_on_worker(store, batch_limits, epoch))
        }
        #[cfg(feature = "fjall")]
        "fjall" => {
            let store = Arc::new(
                bitcoin_rs_storage::FjallStore::open_with_cache(txindex_dir, cache_bytes)
                    .map_err(|e| {
                        TxIndexWorkerError::Storage(bitcoin_rs_storage::StorageError::backend(e))
                    })?,
            );
        ...
        "mdbx" => {
            let store = Arc::new(
                bitcoin_rs_storage::MdbxStore::open_with_cache(txindex_dir, cache_bytes)
                    .map_err(|e| {
                        TxIndexWorkerError::Storage(bitcoin_rs_storage::StorageError::backend(e))
                    })?,
            );
...
{
    let writer = open_writer(&store, epoch).expect("open writer on worker");
