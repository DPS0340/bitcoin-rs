...
struct TxIngressConsumer {
    ...
    mining_control: Arc<dyn MiningControl>,
    peer_outbound:
        Arc<parking_lot::RwLock<hashbrown::HashMap<std::net::SocketAddr, bitcoin_rs_p2p::PeerLease>>>,
}
...
impl TxIngressConsumer {
    ...
    fn relay_tx(&self, txid: Txid, _wtxid: Wtxid) {
        ...
        let inv = Inventory::Transaction(bitcoin::hashes::Hash::from_byte_array(
            *txid.as_bytes(),
        ));
...
mod tests {
    ...
    fn consumer_preserves_exact_connection_id() {
        ...
        if let Some(bitcoin_rs_mempool::AdmissionOrigin::Peer(token)) = origin {
            assert_eq!(
                token.connection_id, expected_conn_id.get(),
