        ...
        while let Some(op) = ops.next() {
            match op.cf() {
                ColumnFamily::TxConfirmed => {
                    apply_fixed_run(&write_txn, TXINDEX_TX_CONFIRMED, op, &mut ops)?;
                }
                ColumnFamily::Funding => {
                    apply_fixed_run(&write_txn, TXINDEX_FUNDING, op, &mut ops)?;
                }
...
impl KvStore for RedbTxIndexStore {
    ...
    fn get(&self, cf: ColumnFamily, key: &[u8]) -> Result<Option<Vec<u8>>, StorageError> {
        let read_txn = self.db.begin_read().map_err(StorageError::backend)?;
        match cf {
            ColumnFamily::TxConfirmed => fixed_get(&read_txn, TXINDEX_TX_CONFIRMED, key),
            ColumnFamily::Funding => fixed_get(&read_txn, TXINDEX_FUNDING, key),
...
impl KvSnapshot for RedbTxIndexSnapshot {
    fn get(&self, cf: ColumnFamily, key: &[u8]) -> Result<Option<Vec<u8>>, StorageError> {
        match cf {
            ColumnFamily::TxConfirmed => fixed_get(&self.read_txn, TXINDEX_TX_CONFIRMED, key),
            ColumnFamily::Funding => fixed_get(&self.read_txn, TXINDEX_FUNDING, key),
...
fn collect_txindex_prefix(
    ...
    match cf {
        ColumnFamily::TxConfirmed => {
            fixed_prefix_collect::<12>(read_txn, TXINDEX_TX_CONFIRMED, prefix)
        }
        ColumnFamily::Funding => fixed_prefix_collect::<12>(read_txn, TXINDEX_FUNDING, prefix),
...
fn scan_txindex_prefix(
    ...
    match cf {
        ColumnFamily::TxConfirmed => {
            fixed_prefix_scan::<12>(read_txn, TXINDEX_TX_CONFIRMED, prefix, limit)
        }
        ColumnFamily::Funding => fixed_prefix_scan::<12>(read_txn, TXINDEX_FUNDING, prefix, limit),
        ColumnFamily::Spending => {
