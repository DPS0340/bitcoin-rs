...
#[test]
fn target_pruning_keeps_bodies_above_the_durable_recovery_base()
-> Result<(), Box<dyn std::error::Error>> {
    let store = MemoryStore::default();
    let data_dir = tempdir()?;
    let block_files = FlatFileBlockStore::open(data_dir.path())?;
    let low_hash = fake_hash(100);
    let high_hash = fake_hash(113);
    let low_position = block_files.append(100, *low_hash.as_byte_array(), b"low body")?;
    let high_position = block_files.append(113, *high_hash.as_byte_array(), b"high body")?;
    let mut initial_batch = store.new_batch();
    initial_batch.put(
        BLOCK_DATA_CF,
        &block_body_key(100, low_hash),
        &low_position.encode(),
    );
    initial_batch.put(
        BLOCK_DATA_CF,
        &block_body_key(113, high_hash),
        &high_position.encode(),
    );
    store.write(initial_batch)?;

    // The policy's retention depth is 288, so the durable recovery horizon is
    // 400 - 288 = 112. A high in-memory tip must not lower that horizon.
    let mut prune_batch = store.new_batch();
    ...
    assert!(undo_outcome.is_empty());
    assert!(file_numbers.is_empty());
    assert!(prune_batch.ops.iter().any(|op| {
        matches!(
            op,
            MemoryOp::Delete { cf: BLOCK_DATA_CF, key }
                if key == &block_body_key(100, low_hash)
        )
    }));
    assert!(!prune_batch.ops.iter().any(|op| {
        matches!(
            op,
            MemoryOp::Delete { cf: BLOCK_DATA_CF, key }
                if key == &block_body_key(113, high_hash)
        )
    }));
    Ok(())
}
