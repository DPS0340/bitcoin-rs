...
fn txindex_invalid_operation_aborts_transaction() -> TestResult<()> {
    ...
    let mut batch = store.new_batch();
    batch.put(ColumnFamily::TxConfirmed, &valid, b"");
    batch.put(ColumnFamily::Funding, &key_12(2), b"non-empty");
    assert_invalid(store.write(batch));
    assert!(store.get(ColumnFamily::TxConfirmed, &valid)?.is_none());
    assert!(store.get(ColumnFamily::Funding, &key_12(2))?.is_none());
...
fn txindex_snapshot_isolation() -> TestResult<()> {
    ...
    let k1 = key_12(1);
    store.put(ColumnFamily::TxConfirmed, &k1, b"")?;

    let snapshot = store.snapshot()?;

    let k2 = key_12(2);
    store.put(ColumnFamily::TxConfirmed, &k2, b"")?;

    // The original snapshot sees only k1.
    let snap_scan = snapshot.scan_prefix_bounded(ColumnFamily::TxConfirmed, &[], MAX_SCAN)?;
    assert_eq!(snap_scan.rows.len(), 1);
    assert!(snap_scan.rows.iter().any(|(k, _)| k == &k1));
    assert!(snap_scan.rows.iter().all(|(k, _)| k != &k2));

    // A new snapshot sees both k1 and k2.
    let fresh = store.snapshot()?;
    let fresh_scan = fresh.scan_prefix_bounded(ColumnFamily::TxConfirmed, &[], MAX_SCAN)?;
    assert_eq!(fresh_scan.rows.len(), 2);

    // Reusing the original snapshot again still returns the old view.
    let snap_scan2 = snapshot.scan_prefix_bounded(ColumnFamily::TxConfirmed, &[], MAX_SCAN)?;
    assert_eq!(snap_scan2.rows.len(), 1);
