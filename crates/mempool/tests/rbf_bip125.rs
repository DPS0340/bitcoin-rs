...
fn rule_one_requires_every_conflict_to_opt_in() -> Result<(), Box<dyn Error>> {
    for (name, second_sequence, expected) in [
        ...
        let opt_in = tx_from_inputs(20, &[(first_input, Sequence::ENABLE_RBF_NO_LOCKTIME)], 1);
        ...
        let replacement = tx_from_inputs(
            40,
            &[
                (first_input, Sequence::ENABLE_RBF_NO_LOCKTIME),
                (second_input, Sequence::ENABLE_RBF_NO_LOCKTIME),
            ],
            1,
        );
