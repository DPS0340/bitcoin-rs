...
///
/// The peer lifecycle owns live leases and ready metadata. Ban policy and
/// network activity are shared inputs supplied by the node.
#[derive(Clone)]
...
fn register_peer(
    ...
    let replaced = peer_lifecycle.register(peer_addr, lease);
    publish_peer_info(peer_lifecycle, peer_ready, peer_addr, lease, info);
    replaced
}
...
pub fn serve_with_shutdown_with_lifecycle_and_chain_and_sync_wake(
    ...
    let shared = ConnectionShared::from_lifecycle(
        peer_lifecycle,
        banned,
        network_active,
        chain_query,
    ...
    while !shutdown.load(Ordering::Relaxed) {
        ...
        match listener.accept() {
            Ok((stream, peer_addr)) => {
                ...
                if !shared.network_active.load(Ordering::Acquire) {
                    drop(stream);
...
fn run_outbound_connection(
    ...
    shared.peer_lifecycle.register(addr, &lease);
    ...
    if let Err(error) = run_outbound_handshake(&mut peer, nonce, 0, &lease, handshake_deadline) {
        shared.peer_lifecycle.remove_current(addr, &lease);
...
fn run_outbound_handshake<S: std::io::Read + std::io::Write>(
    ...
    for message in outbound_messages {
        crate::handshake::send_handshake_message(peer, &message)?;
    }
    ...
    while peer.state != crate::peer::PeerState::Ready {
        ...
        for response in responses {
            crate::handshake::send_handshake_message(peer, &response)?;
        }
    }
...
fn run_handshake(
    ...
    if let Err(error) = run_inbound_handshake(&mut peer, nonce, 0, &lease, handshake_deadline) {
        shared.peer_lifecycle.remove_current(peer_addr, &lease);
        let _ = peer.stream.shutdown(std::net::Shutdown::Both);
        if lease.is_cancelled() {
...
fn run_connected_session(
    ...
    let setup_result: Result<std::thread::JoinHandle<()>, crate::wire::PeerError> = (|| {
        ...
        let writer_stream = peer
            ...
            .try_clone()
            .map_err(crate::wire::PeerError::Io)?;
        spawn_connection_writer(writer_stream, magic, outbound_rx, peer_addr)
            .map_err(crate::wire::PeerError::Io)
    })();
    let writer = match setup_result {
        Ok(handle) => handle,
        Err(error) => {
            // The lease was already registered into the peer lifecycle at
    ...
    let published = publish_peer_info(
        &shared.peer_lifecycle,
        shared.peer_ready.as_deref(),
        peer_addr,
        &lease,
        info,
    );
    ...
    let removed_current = shared.peer_lifecycle.remove_current(peer_addr, &lease);
    debug_assert!(
        removed_current
            || shared
                .peer_lifecycle
...
mod lease_tests {
    ...
    struct ReplacingStream {
        lifecycle: Arc<crate::PeerLifecycle>,
        addr: SocketAddr,
    ...
    fn message_loop_exits_after_replacement_during_read() {
        ...
        let lifecycle = Arc::new(crate::PeerLifecycle::new(
            Arc::clone(&registry),
            Arc::clone(&outbound),
        ));
    ...
    fn registration_cancels_replaced_lease() {
        ...
        let registry = Arc::new(RwLock::new(vec![peer_info(addr, 1)]));
        let outbound = Arc::new(RwLock::new(hashbrown::HashMap::new()));
        let lifecycle = crate::PeerLifecycle::new(Arc::clone(&registry), Arc::clone(&outbound));
        let (old_tx, _old_rx) = crossbeam_channel::unbounded();
        let old = crate::PeerLease::new(old_tx);
        outbound.write().insert(addr, old.clone());
        let (replacement_tx, _replacement_rx) = crossbeam_channel::unbounded();
    ...
    fn stale_release_preserves_replacement_and_current_release_removes_it() {
        ...
        let registry = Arc::new(RwLock::new(vec![peer_info(addr, 2)]));
        let outbound = Arc::new(RwLock::new(hashbrown::HashMap::new()));
        let lifecycle = crate::PeerLifecycle::new(Arc::clone(&registry), Arc::clone(&outbound));
        ...
        assert!(!lifecycle.remove_current(addr, &stale));
        ...
        assert!(lifecycle.remove_current(addr, &current));
        assert!(current.is_cancelled());
...
mod resilient_accept_tests {
    ...
    fn shared_state() -> ConnectionShared {
        ...
        let banned = Arc::new(RwLock::new(Vec::new()));
        let lifecycle = Arc::new(crate::PeerLifecycle::new(peer_registry, leases));
        ConnectionShared::from_lifecycle(
            lifecycle,
            banned,
            Arc::new(AtomicBool::new(true)),
            None,
...
mod writer_setup_cleanup_tests {
    ...
    fn writer_setup_failure_cleans_up_lease() {
        ...
        assert!(
            ...
            "peer lifecycle must be cleaned up after writer setup failure"
        );
    }
}
