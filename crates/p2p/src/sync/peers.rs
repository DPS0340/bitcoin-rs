...
impl BlockSync {
    ...
    pub fn on_peer_ready(&self, source: crate::PeerSource) {
        ...
        self.peer_table.with_current(source, || {
            ...
            // A deferred gap recovery is owed only to the connection that
            // delivered the gapped body; a replaced or departed deliverer
            // leaves no retry.
            if scheduler
                .deferred_gap_recovery
                .is_some_and(|(deferred_source, _)| deferred_source.addr == source.addr)
            {
                scheduler.deferred_gap_recovery = None;
            }
        });
    }
    ...
    pub(super) fn reconcile_peer_sessions(&self) {
        ...
        // Same ownership rule for a recovery deferred on the request slot:
        // only the exact delivering connection can answer it.
        if scheduler.deferred_gap_recovery.is_some_and(|(source, _)| {
            !live
                .iter()
                .any(|(addr, id)| *addr == source.addr && *id == source.connection_id())
        }) {
            scheduler.deferred_gap_recovery = None;
        }
