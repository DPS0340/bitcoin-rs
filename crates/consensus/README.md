...
`connect_block_dual_path` connects a block through both the kernel and `RustValidator`
and returns the kernel verdict, logging any disagreement between the two.
...
script preparation reuses; the portable side is `RustValidator` over a `UtxoView`,
producing a `BlockState` from the previous `TipState`. BIP9 activation is `compute_state`
