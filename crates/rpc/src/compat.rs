...
pub enum Status {
    ...
    /// Refused by product policy, with a stable error.
    Disabled,
...
mod tests {
    ...
    /// The dispatcher's own source, embedded for the same reason the manifest
    /// is: the check has to run against the code that shipped, not against
    /// whatever happens to be on disk when the test runs.
    const DISPATCHER_SOURCE: &str = include_str!("handlers.rs");
    ...
    /// Method names the dispatcher actually answers, read out of its own source.
    ///
    /// Parsed rather than enumerated from a list, because a list is one more
    /// thing that can be forgotten. The match arms are the truth about what
    /// dispatches; this reads them, and the two assertions below make the
    /// manifest agree with them in both directions.
    fn dispatched_methods() -> (Vec<String>, Vec<String>) {
        let body = {
            let start = DISPATCHER_SOURCE
                .find("pub fn dispatch")
                .unwrap_or_else(|| panic!("the dispatcher must still be called `dispatch`"));
            let end = DISPATCHER_SOURCE
                .find("_ => Err(RpcError::MethodNotFound")
                .unwrap_or_else(|| panic!("the dispatcher must still have a fallthrough arm"));
            assert!(
                start < end,
                "the fallthrough arm must follow the dispatcher"
            );
            &DISPATCHER_SOURCE[start..end]
        };
        let split = body
            .find("Err(RpcError::method_disabled")
            .map_or(body.len(), |disabled_arm| {
                // The disabled names are the ones listed above the arm that
                // refuses them, back to the previous dispatching arm.
                body[..disabled_arm]
                    .rfind("(&self.ctx, params),")
                    .map_or(0, |last| last + "(&self.ctx, params),".len())
            });
        let (live_src, disabled_src) = body.split_at(split);
    ...
    fn the_manifest_and_the_dispatcher_name_the_same_methods() {
        let (live, disabled) = dispatched_methods();
        assert!(
            live.len() > 40 && disabled.len() > 5,
            "the source scan found {} live and {} disabled methods, which does not \
             look like this dispatcher — the parse, not the manifest, is what broke",
            live.len(),
            disabled.len()
        );

        let mut dispatched: Vec<String> = live.iter().chain(disabled.iter()).cloned().collect();
        dispatched.sort_unstable();

        let table = manifest();
        let mut listed: Vec<String> = entries(&table, "rpc")
            .iter()
    ...
    /// Every method the manifest calls disabled is refused as disabled.
    ///
    /// A stable refusal is the product decision — this node holds no private
    /// keys — and it is worth a caller being able to tell it apart from a
    /// method that simply does not exist. So the assertion is on the error
    /// kind, not merely on the call failing.
    #[test]
    fn a_disabled_method_is_refused_rather_than_missing() {
        ...
        for entry in entries(&table, "rpc") {
            if status_of(&entry) != Status::Disabled {
                continue;
            }
            ...
            assert!(
                ...
                "`{method}` is listed disabled but answered {outcome:?}"
            );
            checked = checked.saturating_add(1);
        }
        assert!(checked > 0, "the manifest must list the disabled methods");
    }
    ...
    fn every_listed_method_dispatches() {
        ...
        for entry in entries(&table, "rpc") {
            ...
            if matches!(status, Status::Disabled | Status::NotImplemented) {
                continue;
            }
            let method = field(&entry, "method");
            let outcome = handler.dispatch(&method, &json!([]));
            ...
            assert!(
                !matches!(outcome, Err(RpcError::MethodDisabled(_))),
                "`{method}` is listed as {status:?} but the node refuses it as                  disabled — the manifest is advertising a capability that is not there"
            );
        }
    }
