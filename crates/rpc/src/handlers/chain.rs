...
mod stripped_size_tests {
    ...
    fn getblock_reports_a_strippedsize_below_size_for_a_witness_block() {
        ...
        let ctx = Arc::new(Context::new());
        // The log holds no header: records carry one only on the way out to a
        // caller, resolved from the tree. So the tree is where a fixture puts
        // it, or `getblock` answers from the synthetic path and every
        ...
        // Height zero, matching the root the tree insert above made it: the
        // record and the tree node are joined by height, and a mismatch sends
        // the lookup down the header-only path with no body behind it.
        ctx.add_block(BlockRecord::from_block_bytes(0, &block, &bytes));
